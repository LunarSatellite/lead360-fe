...
function Inner({ data }: { data: FlowNodeData }) {
  ...
  return (
    <div className="relative" style={{ paddingTop: issue ? '22px' : '0' }}>
      ...
      <Handle type="target" position={Position.Top} className="!w-3 !h-3 !bg-amber-500 !border-2 !border-bg-card" />
        ...
        <div className="px-3 py-2">
          {rules.length > 0 ? rules.map((r, i) => <div key={i} className="text-[10px] text-amber-400 py-0.5">→ {r}</div>) : <span className="px-1.5 py-0.5 rounded bg-amber-950 text-amber-400 text-[10px] font-medium">Branch</span>}
        </div>
        <Handle type="source" position={Position.Bottom} className="!w-3 !h-3 !bg-amber-500 !border-2 !border-bg-card" />
      </div>
    </div>
  );
}
