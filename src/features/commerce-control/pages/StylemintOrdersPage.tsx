...
const NEXT_ACTION: Record<number, { command: string; label: string }> = {
  2: { command: 'accept', label: 'Accept' },
  3: { command: 'accept', label: 'Accept' },
  12: { command: 'packed', label: 'Mark packed' },
  13: { command: 'ready-to-ship', label: 'Ready to ship' },
  4: { command: 'in-transit', label: 'Mark in transit' },
  6: { command: 'in-transit', label: 'Mark in transit' },
  14: { command: 'in-transit', label: 'Mark in transit' },
  10: { command: 'out-for-delivery', label: 'Out for delivery' },
  11: { command: 'delivered', label: 'Confirm delivered' },
};
...
export function StylemintOrdersPage() {
    ...
    onSuccess: async (_data, action) => {
      setBulkMessage(
        action === 'packing-slips'
          ? 'Labels generated by Stylemint.'
          : 'Bulk action applied successfully.',
      );
      setSelectedIds([]);
      await client.invalidateQueries({ queryKey: ['stylemint-orders'] });
    },
    onError: () => setBulkMessage('Bulk action failed. Check the selected statuses.'),
  });
  const sealPackage = useMutation({
    ...
    onSuccess: async () => {
      setBulkMessage('Security seal recorded in Stylemint.');
      setSealOpen(false);
      await client.invalidateQueries({ queryKey: ['stylemint-orders'] });
    },
  });
    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <aside className="relative h-full w-full max-w-lg overflow-y-auto border-l border-border-subtle bg-bg-card p-6 shadow-2xl">
        ...
        <div className="mt-6 grid grid-cols-2 gap-3">
          ...
          <Info label="Transporteur" value={order.carrier || 'Unassigned'} />
          <Info label="Tracking" value={order.trackingNumber || 'Unavailable'} />
        </div>
        {detailLoading && (
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="mt-5 space-y-3">
          <Input label="Seal identifier" value={sealId} onChange={setSealId} />
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      >
        ...
        <p className="mt-2 text-xs leading-5 text-text-muted">
          {isReject
            ? 'The reason is recorded in Stylemint and any applicable refund is started.'
            : 'This is synchronised with Stylemint immediately.'}
        </p>
        <div className="mt-5 space-y-3">
          {isReject && (
            <label className="block space-y-1.5">
              ...
              >
                ...
                <option value="4">Address not served</option>
          ...
          <Input
            label={isReject ? (reasonCode === '6' ? 'Explanation required' : 'Note to the customer (optional)') : 'Handover note (optional)'}
            value={note}
            onChange={setNote}
      ...
      >
        ...
        {context.data?.paymentIntentId && (
          <label className="mt-4 flex items-start gap-3 rounded-xl border border-danger/20 bg-danger/5 p-3 text-xs leading-5 text-text-secondary">
            ...
            />
            <span>
              Je confirme le remboursement de <strong>{Number(amount || 0).toLocaleString('fr-CD')} CDF</strong> for order <strong>{context.data.orderNumber}</strong>. Cette operation financiere sera auditee.
            </span>
          </label>
        )}
        {refund.isError && (
