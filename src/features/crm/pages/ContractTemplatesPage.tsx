        ...
        <div className="flex flex-col gap-2">
          ...
          : templates.length === 0 ? (
            ...
            <button key={t.id} onClick={() => { setSelectedId(t.id); setShowForm(false); }}
              ...
              <div className="text-[11px] text-text-muted mt-1">{CONTRACT_TEMPLATE_CATEGORY_LABELS[t.category as ContractTemplateCategoryValue] ?? 'General'}{t.isActive === false ? ' · inactive' : ''}</div>
            </button>
          ))}
        </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between gap-3">
                <div><h2 className="text-base font-bold text-text-primary">{selected.name}</h2>
                  <p className="text-xs text-text-muted">{CONTRACT_TEMPLATE_CATEGORY_LABELS[selected.category as ContractTemplateCategoryValue] ?? 'General'}</p></div>
