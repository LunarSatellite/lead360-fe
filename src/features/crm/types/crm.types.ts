...
// ─── Shared inbox, audit and notification preferences (AfterSales) ────────────
//
// These mirror the DTOs the API actually returns. They were referenced by the AfterSales pages
// but never declared, so those pages did not compile on that branch either; the shapes below
// come from the controllers and DTOs in lead360, not from what the pages assumed.

/** What kind of record an inbox row is. Matches CrmInboxItemKind on the API. */
export enum CrmInboxItemKind {
  Lead = 1,
  SupportCase = 2,
  Task = 3,
  Return = 4,
...
export const CRM_INBOX_KIND_LABELS: Record<number, string> = {
  [CrmInboxItemKind.Lead]: 'Lead',
  [CrmInboxItemKind.SupportCase]: 'Support case',
  [CrmInboxItemKind.Task]: 'Task',
  [CrmInboxItemKind.Return]: 'Return',
  [CrmInboxItemKind.WorkOrder]: 'Work order',
  [CrmInboxItemKind.Onboarding]: 'Onboarding',
};

export interface CrmInboxItemDto {
  kind: CrmInboxItemKind;
  id: string;
  title: string;
  status?: string | null;
...
/** Matches CrmAuditOperation on the API. */
export enum CrmAuditOperation {
  Created = 1,
  Updated = 2,
  Deleted = 3,
}

export const CRM_AUDIT_OPERATION_LABELS: Record<number, string> = {
  [CrmAuditOperation.Created]: 'Created',
  [CrmAuditOperation.Updated]: 'Updated',
  [CrmAuditOperation.Deleted]: 'Deleted',
};

/** One field change. The audit trail is per-field, not per-record. */
...
/** Matches NotificationDeliveryChannel on the API. */
...
/**
 * Matches NotificationDigestMode. `Off` and `Disabled` are the same value (3) on the API; only
 * one name is carried here so a switch cannot appear to handle two cases that are one.
 */
...
/** The notification types a user can configure. Matches NotificationType on the API. */
export enum CrmNotificationType {
  ...
  CommentMention = 8,
  TaskAssigned = 9,
  OnboardingAssigned = 10,
  ReturnAssigned = 11,
  WorkOrderAssigned = 12,
}

/**
 * The types the settings screen offers. Every value of the enum is configurable today, but the
 * screen iterates this list rather than the enum so that adding a type to the API does not
 * silently add an unlabelled row to someone's settings.
 */
export const CONFIGURABLE_NOTIFICATION_TYPES: Array<{
  value: CrmNotificationType;
  label: string;
  description: string;
}> = [
  { value: CrmNotificationType.HotLead, label: 'A lead turns hot', description: 'Scoring crossed the hot threshold, so the lead is worth calling now.' },
  { value: CrmNotificationType.LeadConverted, label: 'A lead converts', description: 'A lead became a customer.' },
  { value: CrmNotificationType.CampaignComplete, label: 'A campaign finishes', description: 'Every recipient has been processed.' },
  { value: CrmNotificationType.NurtureReply, label: 'Someone replies to nurture', description: 'A contact answered a nurture message, so a human should take over.' },
  { value: CrmNotificationType.LeadAssigned, label: 'A lead is assigned to me', description: 'Someone put a lead in your name.' },
  { value: CrmNotificationType.SlaBreached, label: 'An SLA is breached', description: 'A case passed the time it was promised in.' },
  { value: CrmNotificationType.Announcement, label: 'An announcement is posted', description: 'A team-wide notice was published.' },
  { value: CrmNotificationType.CommentMention, label: 'I am mentioned in a comment', description: 'Someone typed your name into a comment.' },
  { value: CrmNotificationType.TaskAssigned, label: 'A task is assigned to me', description: 'A task was put in your name.' },
  { value: CrmNotificationType.OnboardingAssigned, label: 'An onboarding is assigned to me', description: 'A customer onboarding was put in your name.' },
  { value: CrmNotificationType.ReturnAssigned, label: 'A return is assigned to me', description: 'A return was put in your name.' },
  { value: CrmNotificationType.WorkOrderAssigned, label: 'A work order is assigned to me', description: 'A work order was put in your name.' },
];

export const NOTIFICATION_TYPE_LABELS: Record<number, string> = {
  [CrmNotificationType.HotLead]: 'A lead turns hot',
  [CrmNotificationType.LeadConverted]: 'A lead converts',
  [CrmNotificationType.CampaignComplete]: 'A campaign finishes',
  [CrmNotificationType.NurtureReply]: 'Someone replies to nurture',
  [CrmNotificationType.LeadAssigned]: 'A lead is assigned to me',
  [CrmNotificationType.SlaBreached]: 'An SLA is breached',
  [CrmNotificationType.Announcement]: 'An announcement is posted',
  [CrmNotificationType.CommentMention]: 'I am mentioned in a comment',
  [CrmNotificationType.TaskAssigned]: 'A task is assigned to me',
  [CrmNotificationType.OnboardingAssigned]: 'An onboarding is assigned to me',
  [CrmNotificationType.ReturnAssigned]: 'A return is assigned to me',
  [CrmNotificationType.WorkOrderAssigned]: 'A work order is assigned to me',
};

export const NOTIFICATION_CHANNEL_LABELS: Record<number, string> = {
  [NotificationDeliveryChannel.InApp]: 'In app',
  [NotificationDeliveryChannel.Email]: 'Email',
  [NotificationDeliveryChannel.Both]: 'In app and email',
};

export const NOTIFICATION_DIGEST_LABELS: Record<number, string> = {
  [NotificationDigestMode.Instant]: 'As it happens',
  [NotificationDigestMode.DailyDigest]: 'Daily digest',
  [NotificationDigestMode.Off]: 'Off',
};

// --- Activity events ---------------------------------------------------------
//
// Written by ICrmActivityWriter on the API. These mirror its enums exactly.

/** What happened. Matches CrmActivityEventKind on the API. */
export enum CrmActivityEventKind {
  CommentAdded = 1,
  StageChanged = 2,
  DealCreated = 3,
  AssignmentChanged = 4,
  ...
  OrderFulfilled = 22,
  OrderCancelled = 23,
  OrderPaymentReceived = 24,
  PurchaseOrderCreated = 25,
  PurchaseOrderApproved = 26,
  PurchaseOrderSentToVendor = 27,
  GoodsReceiptConfirmed = 28,
  SupplierInvoiceReceived = 29,
  SupplierInvoicePaid = 30,
  ReturnCreated = 31,
  ReturnApproved = 32,
  ReturnRejected = 33,
  ReturnReceived = 34,
  ...
  OnboardingMilestoneCompleted = 39,
  OnboardingCompleted = 40,
  OnboardingBlocked = 41,
  WebEventReceived = 42,
}

/** What it happened to. Matches CrmActivityEntityKind on the API. */
export enum CrmActivityEntityKind {
  Contact = 1,
  Deal = 2,
  SupportCase = 3,
  Lead = 4,
...
export const CRM_ACTIVITY_ENTITY_LABELS: Record<number, string> = {
  [CrmActivityEntityKind.Contact]: 'Contact',
  [CrmActivityEntityKind.Deal]: 'Deal',
  [CrmActivityEntityKind.SupportCase]: 'Support case',
  [CrmActivityEntityKind.Lead]: 'Lead',
  [CrmActivityEntityKind.Account]: 'Account',
  [CrmActivityEntityKind.Organization]: 'Organisation',
  [CrmActivityEntityKind.Task]: 'Task',
  [CrmActivityEntityKind.Order]: 'Order',
  [CrmActivityEntityKind.PurchaseOrder]: 'Purchase order',
  [CrmActivityEntityKind.GoodsReceipt]: 'Goods receipt',
  [CrmActivityEntityKind.SupplierInvoice]: 'Supplier invoice',
  [CrmActivityEntityKind.Return]: 'Return',
  [CrmActivityEntityKind.WorkOrder]: 'Work order',
  [CrmActivityEntityKind.Onboarding]: 'Onboarding',
  [CrmActivityEntityKind.WebEvent]: 'Web event',
};

export interface CrmActivityFeedFilter {
  entityKind?: CrmActivityEntityKind;
  eventKind?: CrmActivityEventKind;
  actorUserId?: string;
  page?: number;
  pageSize?: number;
}
...
export interface CrmActivityEventDto {
  id: string;
  entityKind: CrmActivityEntityKind;
  entityId: string;
  eventKind: CrmActivityEventKind;
