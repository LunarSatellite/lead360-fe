...
/**
 * Mentions are stored as a JSON string on the comment, not as an array. A malformed or absent
 * value yields no mentions rather than throwing: a comment is still worth reading without them.
 */
function mentionsOf(comment: { mentionedUserIdsJson: string | null }): string[] {
  if (!comment.mentionedUserIdsJson) return [];
  try {
    const parsed: unknown = JSON.parse(comment.mentionedUserIdsJson);
...
export function CommentsPanel({ kind, entityId }: { kind: number; entityId: string }) {
  ...
  const submit = () => {
    if (!text.trim()) return;
    addComment.mutate(
      { body: text.trim(), mentionedUserIds: mentions },
      {
        onSuccess: () => {
          setText('');
          setMentions([]);
  ...
  // useComments returns a PagedResult; the rows are on .items. Reading .length off the page
  // object itself silently produced undefined before the api client was typed honestly.
  const list = comments && 'items' in comments ? comments.items : [];
            ...
            <div key={c.id} className="flex gap-3">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 text-xs text-text-muted">
                  <span className="font-semibold text-text-primary">{c.authorName || 'Teammate'}</span>
                  <span>· {formatDistanceToNow(parseISO(c.createdAt), { addSuffix: true })}</span>
                  {c.updatedAt && <span>· edited</span>}
                </div>
                <p className="text-sm text-text-secondary whitespace-pre-wrap mt-0.5">{c.body}</p>
                {mentionsOf(c).length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-1">
                    {mentionsOf(c).map((id: string) => (
