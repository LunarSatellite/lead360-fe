...
function Inner({ data }: { data: FlowNodeData }) {
  ...
  return (
    <div className="relative" style={{ paddingTop: issue ? '22px' : '0' }}>
      ...
      <Handle type="target" position={Position.Top} className="!w-3 !h-3 !bg-emerald-500 !border-2 !border-bg-card" />
        <div className="flex items-center gap-2 px-3 py-2.5 border-b border-emerald-800">
          <div className="w-6 h-6 rounded-md bg-emerald-950 flex items-center justify-center text-xs">⚙️</div>
          <span className="text-xs font-semibold text-text-primary truncate flex-1">{data.label}</span>
        </div>
        <div className="px-3 py-2">
          <span className="px-1.5 py-0.5 rounded bg-emerald-950 text-emerald-400 text-[10px] font-medium">Action</span>
        </div>
        <Handle type="source" position={Position.Bottom} className="!w-3 !h-3 !bg-emerald-500 !border-2 !border-bg-card" />
      </div>
    </div>
  );
}

export const ActionNode = memo(Inner);
