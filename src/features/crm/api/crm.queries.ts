...
const CRM_KEYS = {
  ...
  approvals: (status?: number | string) => ['crm', 'approvals', status] as const,
  ...
  recordAudit: (kind: number, entityId: string) => ['crm','record-audit',kind,entityId] as const,
  recordTimeline: (kind: number, entityId: string) => ['crm','record-timeline',kind,entityId] as const,
} as const;
...
export function useReviseQuote() {
  ...
  return useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: CRM_KEYS.quotes() }); toast.success('Quote revised — new draft created.'); },
...
export function useUpdateMeeting() {
  ...
  return useMutation({
    ...
    onSuccess: (_d, vars) => { qc.invalidateQueries({ queryKey: CRM_KEYS.meetingById(vars.id) }); qc.invalidateQueries({ queryKey: CRM_KEYS.meetings() }); toast.success('Meeting updated.'); },
