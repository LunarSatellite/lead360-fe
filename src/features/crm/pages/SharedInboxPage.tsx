...
export function Component() {
  ...
  // The API sends a count per kind, not a total: summing them here keeps this tab agreeing
  // with the six beside it, where a separate total could drift from them.
  const totalUnassigned =
    (summary?.leads ?? 0) +
    (summary?.supportCases ?? 0) +
    (summary?.tasks ?? 0) +
    (summary?.returns ?? 0) +
    (summary?.workOrders ?? 0) +
    (summary?.onboardings ?? 0);
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-8 space-y-6">
      ...
      <div className="flex gap-3 flex-wrap">
        <Stat label="All unassigned" value={totalUnassigned} active={tab === 'all'} onClick={() => setTab('all')} />
